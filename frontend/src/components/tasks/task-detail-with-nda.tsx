...
interface TaskDetailWithNDAProps {
  task: {
    id: string
    title: string
    question: string
...
export function TaskDetailWithNDA({ task, userId }: TaskDetailWithNDAProps) {
  ...
  const handleAcceptNDA = async () => {
    try {
      ...
      // Success - refresh the page to show full task details
  ...
  const handleCloseNDA = () => {
    // Redirect back to task search if user declines NDA
    ...
    <div className="min-h-screen bg-black text-white flex items-center justify-center p-6">
      <div className="max-w-2xl w-full text-center">
        <div className="mb-8">
          ...
          <p className="text-xl text-zinc-400 mb-6">{task.question}</p>
        ...
        <NDADialog
          ...
          ndaText={task.nda_text || '保密协议内容未设置'}
