...
type Task = Database['public']['Tables']['markets']['Row']
...
export function TaskCard({
  ...
  // 计算剩余时间
  const closesAt = new Date(task.closes_at)
  const now = new Date()
  ...
  return (
    <Link href={`/searchs/${task.id}`}>
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-lg p-6 hover:border-emerald-500/30 transition-all group">
        ...
        {/* 问题 */}
        <p className="text-zinc-400 text-sm mb-4 line-clamp-2">{task.question}</p>

        {/* 共识概率 */}
        {consensusProbability !== null && (
          <div className="mb-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-zinc-500 font-mono">任务共识</span>
        ...
        <div className="grid grid-cols-3 gap-4 pt-4 border-t border-zinc-800">
          ...
          <div>
            ...
            <div className="text-lg font-bold text-white">¥{task.reward_pool}</div>
          </div>
          <div>
            <div className="text-xs text-zinc-500 font-mono mb-1">
              {task.status === 'active' ? '剩余时间' : '截止时间'}
            </div>
            {task.status === 'active' && timeRemaining > 0 ? (
