...
export default function MarketCreationForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    console.log('[MarketCreationForm] 开始提交表单')

    if (!validateForm()) {
      console.log('[MarketCreationForm] 表单验证失败', errors)
      return
    }
    ...
    try {
      console.log('[MarketCreationForm] 获取用户 session...')
      // 获取当前用户session
      const { data: { session } } = await supabase.auth.getSession()
      
      if (!session) {
        console.error('[MarketCreationForm] 用户未登录')
        setErrors({ general: '请先登录' })
        return
      }

      console.log('[MarketCreationForm] 用户已认证', {
      ...
      console.log('[MarketCreationForm] 准备调用 API', {
        endpoint: '/api/searchs/create',
        taskType,
        agentCount,
      ...
      console.log('[MarketCreationForm] API 响应状态', {
      ...
      if (!response.ok) {
        console.error('[MarketCreationForm] API 返回错误', {
          status: response.status,
          error: result.error,
          code: result.code,
        })
      ...
      console.log('[MarketCreationForm] 任务创建成功', {
        taskId: result.task_id,
        market: result.market,
      })

      // 成功：跳转到任务详情页
      console.log('[MarketCreationForm] 跳转到任务详情页', {
        url: `/searchs/${result.task_id}`,
      })
      router.push(`/searchs/${result.task_id}`)
    } catch (error) {
      console.error('[MarketCreationForm] 任务创建失败', {
      ...
      console.log('[MarketCreationForm] 表单提交流程结束')
    }
  }
