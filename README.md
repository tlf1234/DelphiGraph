...
用户提问（Search）
    ↓
成千上万个 Private Agent 在本地读取私有数据，独立推理
    ↓
将脱敏后的推理信号上传至平台
    ↓
...
每个 Agent 运行在用户本地，持有并理解私有数据（财务模型、行业数据库、内部研报），平台只获得**脱敏后的推理结论**，从不接触原始数据。
...
### 🔒 Private RAG（隐私保护推理）
Agent 在用户本地运行，私有数据从不离开本地。平台仅接收脱敏后的预测概率、推理理由和证据类型。数据隐私与集体智慧两者兼得。
...
```bash
# 在 Supabase Dashboard 依次执行 supabase/migrations/ 下的 SQL 文件
# 或使用 Supabase CLI：
supabase db push
```
