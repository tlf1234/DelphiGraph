...
class BackgroundDaemon:
    ...
    def process_task(self, task: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.info(f"[AgentOracle]   - 理由: {inference_data.get('rationale', 'N/A')[:100]}...")  # Rationale

            self.logger.info(f"[AgentOracle]   - 证据类型: {inference_data.get('evidence_type', 'N/A')}")  # Evidence Type
            ...
            try:
                ...
                # Build signal object from sanitized data

                signal = {

                    "signal_id": f"sig_{task_id[:8]}_{int(datetime.now().timestamp())}",

                    "evidence_type": sanitized_submission.get("evidence_type", "persona_inference"),

                    "source_type": sanitized_submission.get("source_type", "llm_analysis"),

                    "evidence_text": sanitized_submission.get("rationale", sanitized_submission.get("reasoning", "")),

                    "relevance_reasoning": sanitized_submission.get("evidence_text", ""),

                }

                if sanitized_submission.get("relevance_score") is not None:

                    signal["relevance_score"] = sanitized_submission["relevance_score"]

                if sanitized_submission.get("entity_tags"):

                    signal["entity_tags"] = sanitized_submission["entity_tags"]

                if sanitized_submission.get("source_urls"):

                    signal["source_urls"] = sanitized_submission["source_urls"] if isinstance(sanitized_submission["source_urls"], list) else [sanitized_submission["source_urls"]]

                elif sanitized_submission.get("source_url"):

                    signal["source_urls"] = [sanitized_submission["source_url"]]
                ...
                # Add optional user_persona

                if sanitized_submission.get("user_persona"):

                    payload["user_persona"] = sanitized_submission["user_persona"]

                

                self.logger.info("[AgentOracle] ========================================")

                self.logger.info("[AgentOracle] 📤 正在提交信号数据:")  # Submitting Signal

                self.logger.info(f"[AgentOracle]   - 任务 ID: {task_id}")  # Task ID

                self.logger.info(f"[AgentOracle]   - 信号数: {len(payload['signals'])}")  # Signal count

                self.logger.info(f"[AgentOracle]   - 证据: {signal['evidence_text'][:100]}...")  # Evidence
            ...
            try:
                ...
                try:
                    ...
                    self.submission_logger.log_submission(
                        ...
                        original_submission=inference_data,
...
请开始你的专业分析。"""
    ...
    def _parse_inference_response(self, response_text: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Extract conclusion as rationale
            ...
            self.logger.info(f"[AgentOracle] ⚠️ 使用文本启发式解析: rationale_len={len(rationale)}")
            ...
            return {
                ...
                "evidence_type": "persona_inference",  # default for heuristic
