...
class SubmissionLogger:
    ...
    def log_submission(self, 
        ...
        Args:
            ...
            original_submission: 原始提交数据（脱敏前）

            sanitized_submission: 脱敏后提交数据
        ...
        try:
            ...
            record = {
                ...
                "original_submission": {
                    ...
                    "rationale": original_submission.get("rationale", original_submission.get("reasoning", "")),

                    "evidence_type": original_submission.get("evidence_type", ""),

                    "evidence_text": original_submission.get("evidence_text", ""),

                },

                "sanitized_submission": {

                    "rationale": sanitized_submission.get("rationale", sanitized_submission.get("reasoning", "")),

                    "evidence_type": sanitized_submission.get("evidence_type", ""),

                    "evidence_text": sanitized_submission.get("evidence_text", ""),
    ...
    def _check_if_sanitized(self, original: Dict[str, Any], sanitized: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            original_rationale = original.get("rationale", original.get("reasoning", ""))

            sanitized_rationale = sanitized.get("rationale", sanitized.get("reasoning", ""))

            

            # 比较 evidence_text

            original_evidence = original.get("evidence_text", "")

            sanitized_evidence = sanitized.get("evidence_text", "")

            

            # 如果文本不同，说明被脱敏了

            return (original_rationale != sanitized_rationale) or (original_evidence != sanitized_evidence)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            evidence_types = [s.get("sanitized_submission", {}).get("evidence_type", "") for s in submissions]

            hard_facts = sum(1 for e in evidence_types if e == "hard_fact")
